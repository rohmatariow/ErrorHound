...
class ExtensionTab(ITab, IMessageEditorController):
    ...
    def _create_config_panel(self):
        ...
        title_panel.setMaximumSize(Dimension(999999, 30))
        ...
        subtitle_panel = JPanel(BorderLayout())
        subtitle_panel.setMaximumSize(Dimension(999999, 20))
        subtitle = JLabel("by @rohmatariow")
        subtitle.setFont(Font("Arial", Font.ITALIC, 10))
        subtitle.setForeground(Color(100, 100, 100))
        subtitle_panel.add(subtitle, BorderLayout.WEST)
        ...
        panel.add(self._create_advanced_panel())
        panel.add(Box.createRigidArea(Dimension(0, 15)))
        panel.add(self._create_status_panel())
    ...
    def _create_results_panel(self):
        ...
        title_panel = JPanel(BorderLayout())
        left_panel = JPanel(FlowLayout(FlowLayout.LEFT, 10, 5))
        title = JLabel("Vulnerability Findings")
        title.setFont(Font("Arial", Font.BOLD, 16))
        left_panel.add(title)
        self.stats_label = JLabel("  |  Scans: 0  |  Findings: 0  |  Requests: 0")
        self.stats_label.setFont(Font("Arial", Font.PLAIN, 13))
        self.stats_label.setForeground(Color(100, 100, 100))
        left_panel.add(self.stats_label)
        right_panel = JPanel(FlowLayout(FlowLayout.RIGHT, 5, 5))
        clear_stats_btn = JButton("Clear Statistics", actionPerformed=lambda e: self._clear_stats())
        right_panel.add(clear_stats_btn)
        clear_btn = JButton("Clear Results", actionPerformed=lambda e: self._clear_results())
        right_panel.add(clear_btn)
        
        title_panel.add(left_panel, BorderLayout.WEST)
        title_panel.add(right_panel, BorderLayout.EAST)
        ...
        self.results_table.getColumnModel().getColumn(1).setPreferredWidth(150)
        ...
        self.results_table.getColumnModel().getColumn(4).setPreferredWidth(60)
        self.results_table.getColumnModel().getColumn(5).setPreferredWidth(70)
        self.results_table.getColumnModel().getColumn(6).setPreferredWidth(80)
        self.results_table.getColumnModel().getColumn(7).setPreferredWidth(150)
        self.results_table.getColumnModel().getColumn(8).setPreferredWidth(200)
        self.results_table.getColumnModel().getColumn(9).setPreferredWidth(60)
        self.results_table.getColumnModel().getColumn(10).setPreferredWidth(80)
    ...
    def add_finding(self, finding):
        ...
        try:
            ...
            # # | Host | Method | URL | Status | Length | Severity | Category | Location | Score | Time
            row = [
                str(self.vuln_count),  # #
                host,                  # Host
                method,                # Method
            ...
            def add_row():
                ...
                stats_text = "  |  Scans: {0}  |  Findings: {1}  |  Requests: {2}".format(
                    self.scan_count,
                    self.vuln_count,
                    self.request_count
                )
    ...
    def _clear_results(self):
        ...
        def update_stats():
            stats_text = "  |  Scans: {0}  |  Findings: 0  |  Requests: {1}".format(
                self.scan_count,
                self.request_count
            )
            self.stats_label.setText(stats_text)
    ...
    def _clear_stats(self):
        ...
        def update_stats():
            stats_text = "  |  Scans: 0  |  Findings: {0}  |  Requests: 0".format(self.vuln_count)
            self.stats_label.setText(stats_text)
